...
export class ProductService {
  ...
  getProductList(categoryId: number): Observable<Product[]> {
    const searchUrl = `http://${this.host}/api/products/search/findByCategoryId?id=${categoryId}&projection=productCatalogProjection`;
  ...
  getProductListPaginated(thePage: number,
    ...
    const searchUrl = `http://${this.host}/api/products/search/findByCategoryId?page=${thePage}&size=${thePageSize}&id=${categoryId}&projection=productCatalogProjection`;
    return this.httpClient.get<GetResponseProductCatalog>(searchUrl);
  }

  searchProducts(theKeyword: string): Observable<Product[]> {
    const searchUrl = `http://${this.host}/api/products/search/findByNameContainingIgnoreCase?name=${theKeyword}&projection=productCatalogProjection`;
    return this.getProducts(searchUrl);
  }
  ...
  searchProductsPaginated(thePage: number,
    ...
    const searchUrl = `http://${this.host}/api/products/search/findByNameContainingIgnoreCase?name=${theKeyword}&page=${thePage}&size=${thePageSize}&projection=productCatalogProjection`;
    return this.httpClient.get<GetResponseProductCatalog>(searchUrl);
  }

  getProductDetail(productId: number): Observable<Product> {
    const produtctUrl = `http://${this.host}/api/products/${productId}?projection=productCatalog`;
